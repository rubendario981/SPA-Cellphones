...
export default function Pago() {
  ...
  function handleChangeInput(e) {
    if (e.target.name === "name")
      user.data.findUser.name = e.target.value
    if (e.target.name === "email")
      user.data.findUser.email = e.target.value
        ...
        <form onSubmit={handelSubmit} className="bg-blue-300 w-full h-full">
          <div className="m-60 flex flex-col text-center text-xl">
            ...
            <div className="flex flex-col w-3/5 text-center text-xl justify-center m-auto">
              ...
              <input onChange={handleChangeInput} name="name" type="text" className="text-center text-lg" defaultValue={user?.data?.findUser?.name} />
              <label className="mt-3" htmlFor="">Mail <br /> <p className="text-sm">(Aca va a recibir informacion sobre su pago)</p></label>
              <input onChange={handleChangeInput} name="email" className="mb-5 text-center" type="text" defaultValue={user?.data?.findUser?.email} />
            </div>
