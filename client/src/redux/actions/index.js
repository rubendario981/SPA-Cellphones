...
export const sortReverseName = () => {
	return {
		type: "SORT_REVERSE_NAME"
	};
}
...
export const sortBiggerPrice = () => {
	return {
		type: "SORT_BIGGER_PRICE"
	};
}

export const sortLowerPrice = () => {
	return {
		type: "SORT_LOWER_PRICE"
	};
}
...
export function getProductById(id) {
	return async function (dispatch) {
		try {
			...
			console.log("Response on get product", response);
			return dispatch({
				type: "GET_PHONE_BY_ID",
				payload: response.data
			});
		} catch (error) {
			console.log("Error Redux actions get producto by id", error);
			return error
		}
	};
}
...
export const getLengthCart = () => {
	return dispatch => {
		const lengthCart = JSON.parse(localStorage.getItem("products"));
		return dispatch({
